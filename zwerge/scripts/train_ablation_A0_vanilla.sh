#!/bin/bash
# ============================================================
# Ablation A0: Vanilla Baseline
# ============================================================
# 变量点（全部为原始默认值，无任何改动）：
#   GT_LABEL_TYPE = binary    ← 原始二值 GT
#   FUSION_TYPE   = readiness ← 原始 5特征MLP fusion
#   PROBE_LAYERS  = 18-27     ← 原始层范围
# 作为 A1/A2/A3/A4 的对照组，用于量化每项改进的独立贡献。
# ============================================================

unset http_proxy https_proxy

if [[ -z "${AFO_ENV_CLUSTER_SPEC:-}" ]]; then
    echo "===== [A0-vanilla] DEBUG MODE ====="
...
export WANDB_RUN_NAME="zwerge-A0-vanilla-$(date +%Y%m%d-%H%M%S)"
...
OUTPUT_DIR="/mnt/dolphinfs/ssd_pool/docker/user/hadoop-mt-ocr/yangwenkui03/.hdd/ckpt/zwerge/uitars7b_grounding50k_A0-binary_readiness_L18-27_${RUN_TIMESTAMP}"
mkdir -p "${OUTPUT_DIR}"

echo "WANDB_RUN_NAME = ${WANDB_RUN_NAME}"
echo "OUTPUT_DIR     = ${OUTPUT_DIR}"

# ── 5. 消融参数（vanilla，全部原始默认值）──────────────────────
...
echo "===== [A0-vanilla] Training complete. Output: ${OUTPUT_DIR} ====="
