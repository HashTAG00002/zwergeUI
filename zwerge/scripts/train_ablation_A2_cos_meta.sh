#!/bin/bash
# ============================================================
# Ablation A2: cos_meta fusion only
# ============================================================
# 变量点：
#   GT_LABEL_TYPE = binary    ← 原始二值 GT（对照）
#   FUSION_TYPE   = cos_meta  ← 新 fusion：alpha_l + cos(q_meta, q_l)
#   PROBE_LAYERS  = 18-27     ← 不变
# 对照 base 脚本可以单独观察 cos_meta fusion 的贡献。
# ============================================================

unset http_proxy https_proxy

if [[ -z "${AFO_ENV_CLUSTER_SPEC:-}" ]]; then
    echo "===== [A2-cos_meta] DEBUG MODE ====="
...
export WANDB_RUN_NAME="zwerge-A2-cos_meta-$(date +%Y%m%d-%H%M%S)"
...
OUTPUT_DIR="/mnt/dolphinfs/ssd_pool/docker/user/hadoop-mt-ocr/yangwenkui03/.hdd/ckpt/zwerge/uitars7b_grounding50k_A2-binary_cos_meta_L18-27_${RUN_TIMESTAMP}"
mkdir -p "${OUTPUT_DIR}"
...
FUSION_TYPE="cos_meta"            # ← A2 核心：cos_meta fusion
...
echo "===== [A2-cos_meta] Training complete. Output: ${OUTPUT_DIR} ====="
