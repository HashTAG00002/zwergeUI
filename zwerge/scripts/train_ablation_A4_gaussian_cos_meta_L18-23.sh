...
#   PROBE_LAYERS  = 18-23 (8层)        ← 去掉 L26/L27（最差两层）
...
if [[ -z "${AFO_ENV_CLUSTER_SPEC:-}" ]]; then
    echo "===== [A4-gaussian+cos_meta+L18-23] DEBUG MODE ====="
    ...
    echo "===== [A4-gaussian+cos_meta+L18-23] JOB MODE ====="
    nvidia-smi
...
export WANDB_RUN_NAME="zwerge-A4-gaussian_cos_meta_L18-23-$(date +%Y%m%d-%H%M%S)"
...
OUTPUT_DIR="/mnt/dolphinfs/ssd_pool/docker/user/hadoop-mt-ocr/yangwenkui03/.hdd/ckpt/zwerge/uitars7b_grounding50k_A4-gaussian_cos_meta_L18-23_${RUN_TIMESTAMP}"
mkdir -p "${OUTPUT_DIR}"
...
# ── 5. 消融参数 ─────────────────────────────────────────────────
PROBE_LAYERS="18,19,20,21,22,23"  # ← A4 核心：去掉 L26/L27
...
echo "===== [A4-gaussian+cos_meta+L18-23] Training complete. Output: ${OUTPUT_DIR} ====="
