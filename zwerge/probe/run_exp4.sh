...
# Runtime estimate: ~35 min on 1 GPU (4 models × 150 pairs × 2 passes)
#
# Qwen2.5-VL models (uitars, guiowl7b): qwen25 conda env
# Qwen3-VL models  (guiowl, uivenus):   qwen3-verl conda env
#
# Usage:
#   bash run_exp4.sh [CUDA_ID]
#
# To run a single model:
#   MODEL=uitars bash run_exp4.sh 0
...
run_model() {
    ...
    local CKPT="$2"
    local CONDA_ENV="$3"
    local OUTPUT="${OUT_DIR}/${MODEL_TYPE}_cf.jsonl"
...
echo ""
echo "[exp4] All done. Generating figures …"
