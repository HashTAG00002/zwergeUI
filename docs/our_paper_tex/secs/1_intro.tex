...
\begin{itemize}
  ...
  \item We present \textsc{ZwerGe-UI}, a lightweight frozen-backbone retrofit that recovers intermediate-layer spatial signals through cross-attention probes. It substantially helps Qwen2.5-VL-based agents and spatial/layout tasks, while its benefit on Qwen3-VL coordinate-trained backbones is smaller --- an outcome predicted by the per-backbone severity of the bottleneck (\S\ref{sec:experiments:main}).
\end{itemize}
